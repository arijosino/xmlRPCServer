...
/**
 * A simple implementation of XML-RPC server for apache ws-xmlrpc Version 1.2 -
 * support single instance handler object, but not yet multi-threaded Zen
 * Sugiarto
 */
public class SimpleXmlRpcServer {
	...
	/**
	 * Adds a handler instance (which CAN be stateful) for each request. Note
	 * that every public method in this object will be callable via xml-rpc
	 * client
	 * 
	 * @param name
	 *            - handler name
	...
	/**
	 * Start the xml-rpc server forever. In the rare case of fatal exception,
	 * the web server will be restarted automagically. This is a blocking call
	 * (not thread-based).
	 */
	...
	public SimpleXmlRpcRequestHandlerFactory getHandler() {
		return handler;
	}

	public void setHandler(SimpleXmlRpcRequestHandlerFactory handler) {
